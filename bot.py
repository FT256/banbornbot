...
async def cmd_ban(message: types.Message):
    ...
    if l_name == None:
        await message.answer("Пользователь " + f_name + " (@" + username + ") забанен навсегда!")
    else:
        await message.answer("Пользователь " + f_name + " " + l_name + " (@" + username + ") забанен навсегда!")
...
async def cmd_unban(message: types.Message):
    ...
    username = message.reply_to_message.from_user.username
    if l_name == None:
        await message.answer("Пользователь " + f_name + " (@" + username + ") разбанен")
    else:
        await message.answer("Пользователь " + f_name + " " + l_name + " (@" + username + ") разбанен")
    await message.bot.delete_message(message.chat.id, message.message_id)  # remove admin message
...
async def cmd_readonly(message: types.Message):
    ...
    if len(words) > 1:
        if l_name == None:
            await message.answer(str("Пользователь " + f_name + " (@" + username + ") выдан мут на ({restriction_time})").format(restriction_time=words[1]))
        else:
            await message.answer(str("Пользователь " + f_name + " " + l_name + " (@" + username + ") выдан мут на ({restriction_time})").format(restriction_time=words[1]))
    else:
        if l_name == None:
            await message.answer("Пользователю " + f_name + " (@" + username + ") выдан мут навсегда")
        else:
            await message.answer("Пользователю " + f_name + " " + l_name + " (@" + username + ") выдан мут навсегда")
